...
def get_htf_bias(symbol: str) -> str:
    ...
    """
    try:
        from aion_core.data.kraken_symbol_resolver import resolve_kraken_pair
        kraken_symbol = resolve_kraken_pair(symbol)
        df = fetch_ohlc_data(kraken_symbol, tf="4h", limit=60)  # placeholder
