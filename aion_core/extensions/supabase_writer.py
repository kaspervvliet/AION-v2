...
def log_bias_data(symbol: str, timeframe: str, bias: str, source: str):
    ...
    try:
        ...
        supabase.table("bias_tracker").insert(data)).execute()  # patched
        res = supabase.table("bias_tracker").insert(data).execute()
...
def log_signal_data(symbol: str, setup: dict, rr: float, reason: str):
    ...
    try:
        ...
        supabase.table("signals").insert(data)).execute()  # patched
        res = supabase.table("signals").insert(data).execute()
        logger.info(f"✅ Supabase insert response: {res}")
        logger.info(f"✅ Signal insert response: {res}")
    except Exception as e:
...
def log_reflection_data(setup: dict, decision: str, outcome: str, explanation: str):
    ...
    try:
        ...
        supabase.table("reflections").insert(data)).execute()  # patched
...
def log_equity_data(symbol: str, rr: float, result: float, confidence: float, balance: float):
    ...
    try:
        ...
        supabase.table("equity_log").insert(data)).execute()  # patched
        res = supabase.table("equity_log").insert(data).execute()
        logger.info(f"✅ Supabase insert response: {res}")
    except Exception as e:
