...
def fetch_equity(symbol: str = "SOL/USDT") -> list[dict]:
    """Haalt equity-progressie op uit Supabase."""
    if not supabase:
        logger.warning("❌ Supabase client niet beschikbaar — geen equity opgehaald.")
        return []
    try:
        res = supabase.table("aion_equity").select("*").eq("symbol", symbol).order("id", desc=True).execute()
        return res.data or []
    except Exception as e:
        logger.error(f"❌ Fout bij ophalen equity: {e}")
        return []

def fetch_reflection(symbol: str = "SOL/USDT") -> list[dict]:
    """Haalt AI-reflecties op uit Supabase."""
    if not supabase:
        logger.warning("❌ Supabase client niet beschikbaar — geen reflectie opgehaald.")
        return []
    try:
        res = supabase.table("aion_reflections").select("*").eq("symbol", symbol).order("id", desc=True).execute()
        return res.data or []
    except Exception as e:
        logger.error(f"❌ Fout bij ophalen reflectie: {e}")
        return []
