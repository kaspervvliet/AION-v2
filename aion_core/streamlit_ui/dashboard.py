...
import pandas as pd
import os  # nodig voor pinglog uitlezen
...
with st.sidebar:
    st.markdown("### 🔁 Keep-Alive Monitor")
    if os.path.exists("/tmp/keepalive.log"):
        with open("/tmp/keepalive.log") as f:
            st.text(f.read()[-1000:])  # Laatste 1000 tekens
    else:
        st.info("Geen pinglog gevonden.")
